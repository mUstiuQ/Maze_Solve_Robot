...
const int THINK_TIME = 1000;  // Timp pentru "gândire" (1 secund?)
...
void stopAndThink() {
  ...
  delay(THINK_TIME); // Pauz? de gândire
}
...
void performUTurn() {
  stopAndThink();
  Serial.println("Execut întoarcere complet?");
  motors.setSpeedA(speedA);
  motors.setSpeedB(speedB);
  motors.forwardA();
  motors.backwardB();
...
void loop() {
  ...
  if (frontObstacle) {
    Serial.println("Obstacol detectat în fa??!");
    ...
    if (!rightObstacle && rightDist > leftDist) {
      ...
      Serial.println("Am decis s? virez stânga");
      turnLeft();
    } 
      ...
      Serial.println("Toate c?ile blocate - mers înapoi ?i întoarcere");
      goBackward();
      performUTurn();
    }
  } 
  else {
    // Nicio obstruc?ie - mergem înainte
